...
// Type: AspNetCoreGeneratedDocument.Views_Partials_Forms_Themes_default_MultiPageFormPagingDetails
...
namespace AspNetCoreGeneratedDocument
{
    ...
    RazorPage<FormViewModel>
  {
    ...
    Task ExecuteAsync()
    {
      ...
      stringBuilder.AppendFormat(format, (object) formPagingDetails.Model.PageNumber, (object) formPagingDetails.Model.PageCount);
      ...
      for (int index = 0; index < formPagingDetails.Model.Pages.Count; ++index)
        stringBuilder.AppendFormat("<li class=\"umbraco-forms-paging-captions-caption" + (formPagingDetails.Model.FormStep == index ? " umbraco-forms-paging-captions-caption-current" : string.Empty) + "\">{0}</li>", (object) formPagingDetails.Model.GetPageCaption(index));
      if (formPagingDetails.Model.HasSummaryPage)
        stringBuilder.AppendFormat("<li class=\"umbraco-forms-paging-captions-caption\">{0}</li>", (object) formPagingDetails.Model.SummaryCaption);
      stringBuilder.Append("</ol>");
      formPagingDetails.WriteLiteral("    <div class=\"umbraco-forms-paging\">\n        ");
      formPagingDetails.Write((object) formPagingDetails.Html.Raw(stringBuilder.ToString()));
      formPagingDetails.WriteLiteral("\n    </div>\n");
    }

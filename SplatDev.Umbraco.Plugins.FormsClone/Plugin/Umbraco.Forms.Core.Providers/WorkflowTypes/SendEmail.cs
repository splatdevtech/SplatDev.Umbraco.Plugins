...
// Type: Umbraco.Forms.Core.Providers.WorkflowTypes.SendEmail
    ...
    public class SendEmail : BaseEmailWorkflowType
    {
          ...
          WorkflowExecutionContext context)
        {
            ...
            try
            {
                ...
                while (xpathNodeIterator1.MoveNext() && xpathNodeIterator1.Current != null)
                {
                    XPathNavigator xpathNavigator = xpathNodeIterator1.Current.SelectSingleNode("caption");
                    if (xpathNavigator != null)
                        stringBuilder.AppendFormat("<dt><strong>{0}</strong></dt><dd>", sendEmail._placeholderParsingService.ParsePlaceHolders(sendEmail._dictionaryHelper.GetText(xpathNavigator.Value), false, context));
                    ...
                    while (xpathNodeIterator2.MoveNext() && xpathNodeIterator2.Current != null)
                    {
                        ...
                        DateTime result;
                        if (xpathNodeIterator1.Current.SelectSingleNode("datatype")?.Value == FieldDataType.DateTime.ToString() && DateTime.TryParse(str, out result))
                        ...
                        if (!(sendEmail.Attachment != true.ToString()) && str.Contains("/forms/upload") && sendEmail.TryCreateAttachment(sendEmail._mediaFileManager.FileSystem, str, out attachment))

...
import { jsonStringComparison } from '../../observable-api/index.js';
 ...
 */
export class UmbBaseExtensionInitializer extends UmbControllerBase {
    //
    #promiseResolvers;
    #manifestObserver;
    #extensionRegistry;
    ...
    constructor(host, extensionRegistry, controllerTypeName, alias, onPermissionChanged) {
        super(host, controllerTypeName + '_' + alias);
        //
        this.#promiseResolvers = [];
        this.#overwrites = [];
        this.#conditionControllers = [];
        this.hasConditions = async () => {
        ...
        this.#gotCondition = async (conditionManifest) => {
            ...
            const configsOfThisType = conditionConfigs.filter((conditionConfig) => conditionConfig.alias === conditionManifest.alias);
            // Create conditions, based of condition configs:
            const newConditionControllers = await Promise.all(configsOfThisType.map((conditionConfig) => this.#createConditionController(conditionManifest, conditionConfig)));
        ...
        this.#onConditionsChangedCallback = async () => {
            ...
            if (oldValue !== this.#isPermitted && this.#isPermitted !== undefined) {
                ...
                this.#onPermissionChanged?.(this.#isPermitted, this);
            }
        };
    ...
    _init() {
        this.#manifestObserver = this.observe(this.#extensionRegistry.byAlias(this.#alias), (extensionManifest) => {
    ...
    #cleanConditions() {
        if (this.#conditionControllers === undefined || this.#conditionControllers.length === 0)
            return;
    ...
    #gotManifest(manifest) {
        ...
        */
        ...
        if (conditionConfigs.length > 0) {
            ...
            this.observe(this.#extensionRegistry.byTypeAndAliases('condition', conditionAliases), this.#gotConditions, observeConditionsCtrlAlias);
        }
        else {
            this.removeUmbControllerByAlias(observeConditionsCtrlAlias);
        }
        ...
        if (this.#isPermitted) {
            ...
            const noChangeInConditions = oldAmountOfControllers === this.#conditionControllers.length &&
    ...
    #gotConditions;
    #gotCondition;
    async #createConditionController(conditionManifest, conditionConfig) {
    ...
    */
    #clearPermittedState() {
        if (this.#isPermitted === true) {
            ...
            this._conditionsAreBad();
            this.#onPermissionChanged?.(false, this);
        }
    }
    destroy() {
        if (!this.#extensionRegistry)

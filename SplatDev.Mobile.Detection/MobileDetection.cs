namespace SplatDev.Mobile.Detection
{
    ...
    public static class MobileDetection
    {
            ...
            if (context.Request.ServerVariables["HTTP_USER_AGENT"] != null)
            {
                ...
                foreach (MobileDevice s in MobileDevices)
                {
                    if (context.Request.ServerVariables["HTTP_USER_AGENT"].
                                                        ToLower().Contains(s.Code.ToLower()))
                    {
                        return true;
                    }
                }
        ...
        {
            if (request.Query.ContainsKey("HTTP_X_WAP_PROFILE"))
            {
                return true;
            }
            if (request.Query.ContainsKey("HTTP_ACCEPT"))
            {
                request.Query.TryGetValue("HTTP_ACCEPT", out StringValues values);
                foreach (var val in values)
                {
                    if (val.ToLower() == "wap") return true;
                }
            }
            if (request.Query.ContainsKey("HTTP_USER_AGENT"))
            {

...
{
    using SplatDev.Content.Interfaces;

    using System;

    using SplatDev.Umbraco.Plugins.CodeFirst.Enums;

    public class Asset : ITemplate
    {
        /// <summary>
        ...
        /// <param name="assetType">The type of resource i.e. file, view, css, etc</param>
        ...
        public Asset(string fileName, string outputDirectory, string resourceLocation, AssetTypes assetType, string name = "", string alias = "", bool createBackup = false)
        {
            if (string.IsNullOrEmpty(name) && string.IsNullOrEmpty(alias) && assetType == AssetTypes.Template)
